...
float4 MainPS(VertexShaderOutput input) : COLOR
{
    ...
    float2 translatedPos = lightPosition + translation; //perhapse use this to optomise and realize when light is offscreen by allot? // position of light in screenspace
    ...
    float2 marchDir = normalize(translatedPos - screenCords) * MARCH_STEP; //see if i can just use normalizedDistAxis? should be opisite dir?
    ...
    for (int i = 0; i < MAX_MARCH_STEPS; i++)
    {
        ...
        float sample = tex2D(occluderSampler, marchPos / float2(width*2, height*2) + float2(0.25, 0.25)).r; //see if I can sample without the div? //factor of two is multiplier for occlusiontexxture buffer areas
        if (sample == 0)
        {
            shadowFactor -= 0.25;
        }
