...
float4 MainPS(VertexShaderOutput input) : COLOR
{
    ...
    float waveRippleMask = sin(uv.y * scale * 4 + scaledTime * waveSpeed) + tex2D(noiseMap, uv * scale * float2(0.02, 0.02) + scaledTime).r + hlUVNoise * 2;
    ...
    //float3 waveColor = float3(foam, foam, foam) + float3(0.2, 0.4, 0.9) * (floor(waveNoise * 5) / 8 + 0.3);
    
    return float4(0.5, 0.5, waveRippleMask*0.2+0.8, sample.a); //a is alpha right?
