...
public class GameScene : Scene
{
    ...
    internal const string MAP_WALL_COLLISION_LAYER = "mapWall";
    internal const string ENEMY_COLLISION_LAYER = "enemy";
    internal const string PLAYER_COLLISION_LAYER = "player";
    ...
    private NodePointer MakeLevelBlock(NodePointer parent, CircleCollider col, Vector2 position)
    {
        NodePointer mapNode = this.sceneController.AddNode(parent, "MapGround");

        sceneController.AddBehaviour(mapNode, new Light(new Vector3(200000, 220000, 280000), true, 0, 1));
        col.SetLayer(MAP_FLOOR_COLLISION_LAYER);
    ...
    protected override void OnLoad(SceneManager sceneManager)
    {
        ...
        this.platform1Normal = Game.Content.Load<Texture2D>("Graphics/Platform1Normal");
        this.platform1Roughness = Game.Content.Load<Texture2D>("Graphics/Platform1Roughness");
        #endregion
        ...
        sceneManager.AddBehaviour(gameManager, new GameController());
        #endregion

        #region MAP
        NodePointer mapRoot = sceneManager.AddNodeAtRoot("MapRoot");
        ...
        for (int i = 0; i < 15; i++)
        {
            ...
            radii.Add(radius);
            poses.Add(pos);
            MakeLevelBlock(mapRoot, new CircleCollider(radius), pos);

            if (!(radius > biggestPlat)) continue;
            biggestPlat = radius;
            biggestCentre = pos;
        }

        player.SetLocalPosition(biggestCentre + new Vector2(biggestPlat) * 0.7f);
